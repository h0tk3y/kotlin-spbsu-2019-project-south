...
1. Общее:
    ...
    1. Про UI - ДЛЯ НЕГО ДОДЕЛАЙТЕ Client(а пока Андрей уже делает немного)
    1. **САМОЕ ВАЖНОЕ** Сделать ещё отедльную базу с (login, password, id, jwt) + разделить CHAT НА SINGLE_СHAT И GROUP_CHAT
     (относится ко всем кто работает с DB/Server/ChatData(который также можно разделить, а может и не надо); сделать
     работы с JWT, сделать уровни доступа и фильтр данных на сервере
    1. Доработать сетевую часть(вроде тут Даня)
