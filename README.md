...
1. Общее:
    ...
    1. Про UI - Андрей чо-то делает, см. ветку "console_ui"
    1. Сделать ещё отедльную базу с (login, password, id) + разделить CHAT НА SINGLE_СHAT И GROUP_CHAT
     (относится ко всем кто работает с DB/Server/ChatData(который также можно разделить, а может и не надо) -
      на ближвйшее потом
    1. Ещё начать делать ACCES_LEVEL - чтобы получать только те данные, которые можем получить(например, другой пользователь не должен получать с сервера список контактов/чатов другого)
